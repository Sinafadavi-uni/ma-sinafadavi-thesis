...
def test_emergency_keywords():
    ...
    for wasm, should_flag, typ in test_cases:
        info = JobInfo(
            wasm_bin=wasm,
            capabilities=Capabilities(cpu_cores=1, memory=512, power=5),
            result_addr=Address(host="localhost", port=9000)
        )
...
def test_job_queue_logic():
    ...
    broker = VectorClockExecutorBroker(lambda *_: None)
    reg = JobInfo(
        wasm_bin="regular_task.wasm", 
        capabilities=Capabilities(cpu_cores=1, memory=256, power=10), 
        result_addr=Address(host="localhost", port=9000)
    )
    emg = JobInfo(
        wasm_bin="emergency_fire.wasm", 
        capabilities=Capabilities(cpu_cores=2, memory=1024, power=20), 
        result_addr=Address(host="localhost", port=9000)
    )
    rid, eid = uuid4(), uuid4()
...
def test_executor_capabilities():
    ...
    class FakeExecutor:
        def __init__(self):
            self.id = mock_id
            self.cur_caps = Capabilities(cpu_cores=4, memory=4096, power=80)
            self.last_update = time.time()
            self.submit_job = lambda *_: True

    broker.executors[mock_id] = FakeExecutor()
    low = Capabilities(cpu_cores=1, memory=512, power=10)
    high = Capabilities(cpu_cores=10, memory=16000, power=200)
...
def test_final_object_creation():
    ...
    job_id = uuid4()
    info = JobInfo(
        wasm_bin="end_check.wasm", 
        capabilities=Capabilities(cpu_cores=1, memory=256, power=5), 
        result_addr=Address(host="localhost", port=9000)
    )
